...
class Controller:
    ...
    def handle_controls(gesture, hand_result):  
        ...
        if gesture != Gest.ROCK and Controller.mute_flag:
            Controller.mute_flag = False

        if gesture != Gest.PINKY and Controller.screenshot_flag:
            Controller.screenshot_flag = False

        if gesture != Gest.RING and Controller.lock_flag:
            Controller.lock_flag = False

        if gesture != Gest.THREE_FINGERS and gesture != Gest.LAST3 and Controller.alttab_flag:
            Controller.alttab_flag = False
        ...
        if gesture == Gest.V_GEST:
            ...
            if not Controller.mute_flag:
                pyautogui.press('volumemute')
                Controller.mute_flag = True

        elif gesture == Gest.PINKY:
            if not Controller.screenshot_flag:
                pyautogui.hotkey('win', 'prtscr')
                Controller.screenshot_flag = True

        elif gesture == Gest.RING:
            if not Controller.lock_flag:
                ctypes.windll.user32.LockWorkStation()
                Controller.lock_flag = True

        elif gesture == Gest.THREE_FINGERS or gesture == Gest.LAST3:
            if not Controller.alttab_flag:
                pyautogui.keyDown('alt')
                pyautogui.press('tab')
                pyautogui.keyUp('alt')
                Controller.alttab_flag = True
